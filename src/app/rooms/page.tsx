    ...
    <>
      <section className="relative pt-32 pb-20 sm:pt-40 sm:pb-28 bg-[#1b1b1b] dark:bg-[#0d0d0d]">
        ...
        <div className="container-x relative z-10 text-center">
          ...
          <h1 className="font-display text-4xl sm:text-5xl md:text-6xl text-white mb-4">Rooms &amp; Suites</h1>
          <p className="text-sm text-stone-300 dark:text-stone-400 max-w-xl mx-auto">Twenty-eight appointed residences, each designed for comfort, elegance, and an unforgettable stay in Ado-Ekiti.</p>
        </div>
      </section>
              ...
              <Link key={room.slug} href={`/rooms/${room.slug}`} className="group block border border-[#ece6dd] dark:border-[#2e2b26] bg-white dark:bg-[#1c1a17] overflow-hidden hover:shadow-lg transition-all duration-300 hover:border-[var(--accent)]">
                ...
                <div className="p-6">
                  <div className="flex items-center gap-3 mb-2">
                    <span className="eyebrow text-[var(--accent)]">{room.size}</span>
                    <span className="h-px w-4 bg-[#ece6dd]" />
                    <span className="eyebrow text-stone-400">{room.occupancy}</span>
                  </div>
                  <h2 className="font-display text-xl text-[#222] dark:text-[#f4efe6] mb-2">{room.name}</h2>
                  <p className="text-xs text-[#666] dark:text-[#a8a29e] leading-relaxed mb-4">{room.blurb}</p>
                  <div className="flex items-center justify-between pt-3 border-t border-[#ece6dd] dark:border-[#2e2b26]">
                    <span className="font-display text-xl text-[var(--accent)]">{naira(room.rate)} <span className="text-xs font-condensed text-stone-400">/ night</span></span>
                    <span className="btn-gold px-4 py-2 text-[10px] inline-flex items-center gap-1">View Details <ArrowRight className="h-3 w-3" /></span>
                  </div>
                </div>
              </Link>
            ))}
