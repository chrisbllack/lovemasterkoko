...
import { ArrowRight, Shield, Wifi, Coffee, Utensils, GlassWater, CalendarCheck, Sparkles } from "lucide-react";
          ...
          <div className="lg:col-span-6 space-y-6">
            ...
            <div className="pt-4 flex flex-col lg:flex-row items-center lg:justify-between gap-5 border-t border-[#ece6dd] dark:border-[#2e2b26]">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <span className="font-condensed uppercase tracking-wider text-xs text-[var(--accent)] font-bold block">Reservation Hotline</span>
              ...
              <Link href="/booking" className="btn-gold px-8 py-3.5 text-xs sm:text-sm font-bold inline-flex items-center justify-center text-center mx-auto lg:mx-0 shadow-md">Check Availability</Link>
            </div>
          </div>
          <div className="lg:col-span-6 space-y-4 sm:space-y-6">
            {/* Component Box above image boxes */}
            <div id="component-box" className="component-box rounded-2xl border border-[#ece6dd] dark:border-[#2e2b26] bg-white dark:bg-[#1c1a17] p-5 sm:p-6 shadow-md transition-all">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex items-center gap-3.5">
                  <div className="h-11 w-11 rounded-xl bg-[var(--accent)]/10 text-[var(--accent)] border border-[var(--accent)]/30 flex items-center justify-center shrink-0">
                    <Sparkles className="h-5 w-5" />
                  </div>
                  <div>
                    <span className="eyebrow text-[var(--accent)] block text-[0.68rem] tracking-[0.22em]">Signature Hospitality</span>
                    <h4 className="font-display text-lg sm:text-xl text-stone-900 dark:text-white font-normal leading-snug">
                      Quiet Elegance &amp; Heritage Dining
                    </h4>
                  </div>
                </div>
                <div className="flex items-center gap-2 text-xs font-condensed uppercase tracking-wider text-stone-600 dark:text-stone-300 font-semibold border-t sm:border-t-0 sm:border-l border-[#ece6dd] dark:border-[#2e2b26] pt-3 sm:pt-0 sm:pl-4">
                  <span className="h-2 w-2 rounded-full bg-emerald-500 animate-pulse" />
                  <span>24/7 Power · City Center</span>
                </div>
              </div>
            </div>

            <div className="grid grid-cols-2 gap-4 sm:gap-6 relative">
              <div className="overflow-hidden rounded-2xl border border-[#ece6dd] dark:border-[#2e2b26] shadow-lg h-72 sm:h-96">
                <Image src="/images/hero-exterior.jpg" alt="Hotel exterior" width={400} height={500} className="h-full w-full object-cover hover:scale-105 transition-transform duration-700" sizes="(max-width: 1024px) 50vw, 25vw" />
              </div>
              <div className="overflow-hidden rounded-2xl border border-[#ece6dd] dark:border-[#2e2b26] shadow-lg h-72 sm:h-96">
                <Image src="/images/OpenBar Garden.jpg" alt="Garden bar" width={400} height={500} className="h-full w-full object-cover hover:scale-105 transition-transform duration-700" sizes="(max-width: 1024px) 50vw, 25vw" />
              </div>
            </div>
          </div>
        </div>
    ...
    <section className="py-20 sm:py-28 bg-white dark:bg-[#121212]" style={{ contentVisibility: "auto", containIntrinsicSize: "0 1200px" }}>
      <div className="container-x">
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {ROOMS.slice(0, 8).map((room) => (
            <Link key={room.slug} href={`/rooms/${room.slug}`} className="group block border border-[#ece6dd] dark:border-[#2e2b26] bg-white dark:bg-[#1c1a17] overflow-hidden rounded-2xl hover:shadow-lg transition-all duration-300 hover:border-[var(--accent)]">
              ...
              <div className="p-5">
                <span className="font-condensed text-xs uppercase tracking-wider font-bold text-[var(--accent)] block mb-1">{room.bed} · {room.occupancy}</span>
                <h3 className="font-display font-normal text-xl text-stone-900 dark:text-white mb-2">{room.name}</h3>
                <p className="text-sm text-stone-700 dark:text-stone-200 leading-relaxed line-clamp-2 mb-3 font-medium">{room.blurb}</p>
                <div className="flex items-center justify-between">
                  <span className="font-display text-xl text-[var(--accent)] font-bold">{naira(room.rate)}</span>
                  <span className="text-xs font-condensed uppercase tracking-wider font-bold text-[var(--accent)]">/ night</span>
                </div>
              </div>
            </Link>
          ))}
        ...
        <div className="mt-14 text-center">
          <Link href="/booking" className="btn-gold px-9 py-4 text-xs sm:text-sm font-bold inline-flex items-center gap-2 shadow-md">
      ...
      <div className="container-x">
        ...
        <div className="grid gap-6 sm:gap-8 sm:grid-cols-2 lg:grid-cols-3">
          {FACILITIES.map((f) => {
            const Icon = f.icon;
            return (
              <div key={f.title} className="border border-[#ece6dd] dark:border-[#2e2b26] bg-white dark:bg-[#1c1a17] p-8 transition-all duration-300 hover:border-[var(--accent)] hover:shadow-md rounded">
                <div className="h-14 w-14 border border-[var(--accent)] flex items-center justify-center text-[var(--accent)] mb-6 rounded"><Icon className="h-6 w-6" /></div>
                <h3 className="font-display text-lg sm:text-xl text-stone-800 dark:text-stone-100 font-semibold mb-2">{f.title}</h3>
    ...
    <section className="py-20 sm:py-28 bg-white dark:bg-[#121212]" style={{ contentVisibility: "auto", containIntrinsicSize: "0 700px" }}>
      <div className="container-x">
        ...
        <div className="grid gap-8 md:grid-cols-3">
          {[
            { img: "/images/dining.jpg", title: "Dining: A Feast Steeped in Tradition", tag: "Culinary Art", copy: "Nigerian classics and continental plates, served breakfast through dinner.", href: "/dining" },
            ...
            <Link key={c.title} href={c.href} className="group relative overflow-hidden rounded-2xl border border-[#ece6dd] dark:border-[#2e2b26] block shadow-md">
              ...
              <div className="absolute inset-x-4 bottom-4 border border-white/15 bg-[#0a2777] p-5 text-white rounded-xl">
                <span className="font-condensed text-xs tracking-[0.24em] uppercase text-[var(--accent-light)] font-bold block mb-1">{c.tag}</span>
                <h3 className="text-2xl font-display text-white font-bold">{c.title}</h3>
                <p className="mt-1.5 text-sm text-stone-100 font-medium line-clamp-2">{c.copy}</p>
              </div>
            </Link>
          ))}
        </div>
    ...
    <section className="py-20 sm:py-28 bg-[#f8f5f0] dark:bg-[#1c1a17] border-t border-[#ece6dd] dark:border-[#2e2b26]" style={{ contentVisibility: "auto", containIntrinsicSize: "0 600px" }}>
      <div className="container-x">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {["/images/Hotel Lobby.jpg", "/images/Restaurant 2.jpg", "/images/lounge-bar.jpg", "/images/corridor-hallway.jpg", "/images/dining.jpg", "/images/events-hall.jpg", "/images/lobby.jpg", "/images/room-suite.jpg"].map((img, i) => (
            <div key={i} className={`overflow-hidden rounded-2xl border border-[#ece6dd] dark:border-[#2e2b26] ${i === 0 || i === 5 ? "md:col-span-2 md:row-span-2" : ""}`}>
        ...
        <div className="mt-12 text-center">
          <Link href="/gallery" className="btn-outline-gold px-8 py-3.5 text-xs sm:text-sm font-bold inline-flex items-center gap-2 transition-all">
        ...
        <span className="eyebrow text-[var(--accent-light)] block mb-2">What Our Guests Say</span>        <h2 className="font-display font-normal text-3xl sm:text-4xl md:text-5xl mb-12 text-white">Testimonials</h2>
          ...
          {[
            ...
            <div key={t.name} className="p-8 border border-white/10 bg-white/5 dark:bg-white/[0.03] text-left rounded-2xl">
              ...
              <p className="text-base text-stone-100 font-medium leading-relaxed mb-6 italic">&ldquo;{t.text}&rdquo;</p>
              <span className="font-condensed text-xs uppercase tracking-[0.2em] text-[var(--accent-light)] font-bold">{t.name}</span>
            </div>
          ))}
        </div>
      </div>
    ...
    <section className="py-20 sm:py-28 bg-white dark:bg-[#121212]" style={{ contentVisibility: "auto", containIntrinsicSize: "0 600px" }}>
      <div className="container-x max-w-3xl">
        ...
        <div className="space-y-4">            {FAQS.map((f, i) => (
              <details key={i} className="group border border-[#ece6dd] dark:border-[#2e2b26] p-6">                <summary className="font-display text-lg sm:text-[1.18rem] font-semibold text-stone-800 dark:text-stone-100 cursor-pointer list-none flex items-center justify-between">
                {f.q}
                <span className="text-[var(--accent)] group-open:rotate-45 transition-transform text-xl font-condensed font-semibold">+</span>
  ...
  return (
    <section className="py-20 sm:py-24 bg-blue-900 dark:bg-blue-950 text-white text-center border-t border-[var(--accent)]/30">
      <div className="container-x max-w-2xl mx-auto space-y-5">
        <span className="font-condensed text-xs uppercase tracking-[0.3em] font-bold text-[var(--accent-light)] block">Direct Reservation Privilege</span>
        <h2 className="text-3xl sm:text-5xl font-display font-normal text-white leading-tight">Book Your Next Stay in Ado-Ekiti</h2>
        <p className="text-sm sm:text-base text-stone-100 font-medium leading-relaxed max-w-lg mx-auto">
        ...
        <div className="pt-4 flex flex-wrap items-center justify-center gap-4">
          <Link href="/booking" className="btn-gold px-9 py-4 text-xs sm:text-sm font-bold inline-flex items-center gap-2 shadow-lg">
            <span>Check Availability &amp; Book</span><ArrowRight className="h-4 w-4" />
          </Link>
          <a href={`tel:${HOTEL.phone}`} className="btn-outline-white px-9 py-4 text-xs sm:text-sm font-bold inline-flex items-center gap-2 rounded">
